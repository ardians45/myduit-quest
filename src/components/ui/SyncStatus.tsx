...
export function SyncStatus() {
  ...
  const handleMagicLinkLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.message || 'Gagal mengirim link login.');
    } finally {
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        alert('Sinkronisasi berhasil! Data sudah terupdate.');
      } else {
        alert('Gagal mengambil data dari cloud atau data kosong.');
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden divide-y divide-gray-50 flex flex-col">
      {user ? (
        <>
